...
function displayResults(filteredJobs) {
  ...
  if (filteredJobs.length > 0) {
    filteredJobs.forEach((job, index) => {
      ...
      listItem.innerHTML = `
        <h3><a href="#" id="job-${index}">${job.title}</a></h3>
        <p>${job.location}</p>
      `;
...
document.getElementById('searchForm').addEventListener('submit', (event) => {
  ...
  const filteredJobs = searchJobs(title, location);
...
document.getElementById('searchForm').addEventListener('submit', (event) => {
  event.preventDefault();
 const title = document.getElementById('title').trim;
  const location = document.getElementById('location').trim;
